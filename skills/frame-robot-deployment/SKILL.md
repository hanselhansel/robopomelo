...
Read all specification fields under inspect authority. Write only `project.*`, `stakeholders.*`, `needs.*`, `problems.*`, `challenges.*`, `challengeAnswers.*`. Wildcards do not override immutable IDs or protected-field rules.
Dependencies: none.
Consult the [patch schema](../../packages/spec/schemas/patch-1.0.0.schema.json) and [record schema](../../packages/spec/schemas/need.schema.json) before constructing records. Missing schemas or dependencies are a stop condition.

## Work

Separate the operational problem from a proposed robot solution. Record supplied outcomes, scope exclusions, stakeholders, responsibilities, needs and observed problems. Leave an unknown approver or owner explicit. Use challenges and challengeAnswers for unanswered framing questions. Send flow assumptions to specify-material-flow instead of writing outside this contract.
