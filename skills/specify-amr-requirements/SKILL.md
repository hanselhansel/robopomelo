...
Read all specification fields under inspect authority. Write only `requirements.*`, `risks.*`, `assumptions.*`, `decisions.*`, `challengeAnswers.*`. Wildcards do not override immutable IDs or protected-field rules.
Dependencies: `specify-material-flow`, `define-deployment-kpis`.
Consult the [patch schema](../../packages/spec/schemas/patch-1.0.0.schema.json) and [record schema](../../packages/spec/schemas/requirement.schema.json) before constructing records. Missing schemas or dependencies are a stop condition.

## Work

Write requirements without choosing a vendor. Link rationale to needs, flows and KPIs. Propose risks, mitigations, assumptions and decision questions only from supplied context. Keep decisions proposed. Link a requirement to an existing planned test or record an explicit unresolved verification disposition. Return here after acceptance planning when test IDs become available.
