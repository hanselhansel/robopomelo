import { fixtureEntry } from '../runtime/helpers/entry-path.js';
...
beforeAll(async () => {
  ...
  await build({
    entryPoints: [fixtureEntry('../runtime/helpers/settings-child.ts', import.meta.url)],
    outfile: executable,
    bundle: true,
    platform: 'node',
    format: 'esm',
...
async function fixture() {
  ...
  const trust = new TrustStore(settings);
...
describe('machine-local settings and trust', () => {
  ...
  it('requires explicit manage-settings authority and keeps scopes distinct', async () => {
    ...
    await expect(
      trust.grant(binding, ['author'], 'autonomous', { scopes: ['author'] }),
    ...
    await expect(
      trust.withAuthorization(binding, grant, ['record-decisions'], async () => 'forbidden'),
    ).rejects.toMatchObject({ code: 'SCOPE_DENIED' });
  ...
  it('does not trust copied, moved, replaced or different-ID projects', async () => {
    ...
    await mkdir(copy);
    const copiedRoot = await SafeRoot.open(copy);
    try {
      expect(await trust.lookup({ ...copiedRoot.identity(), projectId: binding.projectId })).toBeUndefined();
    } finally {
      await copiedRoot.close();
    }
  ...
  it('rechecks queued authorization after revocation and retains a tombstone', async () => {
    ...
    await expect(
      trust.withAuthorization(binding, grant, ['author'], async () => 'commit'),
  ...
  it('holds the settings lock through authorized replacement, serializing completed revocation', async () => {
    ...
    const order: string[] = [];
    const commit = trust.withAuthorization(binding, grant, ['author'], async () => {
      enter();
      await held;
      order.push('commit');
    });
    await entered;
    const revoke = trust.revoke(grant.grantId, authority).then(() => {
      order.push('revoke');
    });
    finish();
    await Promise.all([commit, revoke]);
    expect(order).toEqual(['commit', 'revoke']);
    await expect(trust.withAuthorization(binding, grant, ['author'], async () => {})).rejects.toMatchObject({
      code: 'GRANT_REVOKED',
    });
  });
  ...
  it('forgets remembered authority without touching project files', async () => {
    const { trust, binding, project } = await fixture();
    await writeFile(join(project, 'source'), 'project source');
    const grant = await trust.grant(binding, ['author'], 'autonomous', authority);
  ...
  it('preserves prior configuration bytes and ignores an interrupted staged candidate', async () => {
    ...
    if (process.platform !== 'win32')
      expect((await lstat(join(config, 'settings.json'))).mode & 0o777).toBe(0o600);
  });
  it('rejects malformed settings and config links without restoring old authority automatically', async () => {
    ...
    await symlink(config, linked, process.platform === 'win32' ? 'junction' : 'dir');
  ...
  it('requires settings authority for update preferences and preserves invalid-input recovery', async () => {
    ...
    const updates = new UpdatePreferences(settings);
    expect((await updates.read()).mode).toBe('auto');
    await expect(updates.configure({ offline: true }, { scopes: ['author'] })).rejects.toMatchObject({
      code: 'SCOPE_DENIED',
    });
    expect(await updates.configure({ offline: true, pinnedVersion: '1.2.3' }, authority)).toMatchObject({
      offline: true,
      pinnedVersion: '1.2.3',
    });
    await expect(updates.configure({ pinnedVersion: 'latest' }, authority)).rejects.toMatchObject({
      code: 'SETTINGS_INVALID',
    });
    expect((await updates.read()).pinnedVersion).toBe('1.2.3');
  });
  it.runIf(process.platform === 'win32')(
    'compares canonical Windows paths without case-induced false trust changes',
    async () => {
      const { trust, binding } = await fixture();
      const grant = await trust.grant(binding, ['inspect'], 'autonomous', authority);
      expect(await trust.lookup({ ...binding, canonicalPath: binding.canonicalPath.toUpperCase() })).toEqual(
        grant,
      );
    },
  );
