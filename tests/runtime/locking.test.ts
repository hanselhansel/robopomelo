...
beforeAll(async () => {
  ...
  await build({
    entryPoints: [fixtureEntry('./helpers/lock-child.ts', import.meta.url)],
    outfile: executable,
    bundle: true,
    platform: 'node',
    format: 'esm',
...
async function command(process: ChildProcess, command: string): Promise<{ status: string; code?: string }> {
  const result = once(process, 'message');
...
describe('cooperative identity-bound locks', () => {
  it('admits only one of two real OS processes and allows a later owner', async () => {
    ...
    const a = await child(path);
    const b = await child(path);
  ...
  it('recovers a provably dead child under a separate claim and retains quarantine', async () => {
    const { path, root } = await fixture();
    const holder = await child(path);
    expect(await command(holder, 'acquire')).toEqual({ status: 'acquired' });
    await kill(holder);
    const lease = await acquireLock(root);
  ...
  it('arbitrates stale recovery against a fresh competing process', async () => {
    const { path, root } = await fixture();
    const dead = await child(path);
    await command(dead, 'acquire');
    await kill(dead);
    const contender = await child(path);
    const [local, remote] = await Promise.all([
      acquireLock(root).then(
        (lease) => ({ lease }),
        (error) => ({ error }),
      ),
    ...
    const final = await acquireLock(root);
    await final.release();
  });
  it.each(['missing', 'malformed', 'foreign', 'live-old', 'claim'])(
    'never steals an uncertain %s lock',
    async (kind) => {
      const { path, root } = await fixture();
      ...
      if (kind !== 'missing' && kind !== 'claim') {
        ...
        await writeFile(
          join(path, dir, 'owner.json'),
          kind === 'malformed' ? 'not json' : JSON.stringify(owner),
        );
      }
  ...
  it('does not accept caller-selected lock namespaces or remove substituted entries', async () => {
    ...
    const handle = await root.createExclusive('owned');
    await handle.close();
    const identity = await root.stat('owned');
    await rename(join(path, 'owned'), join(path, 'saved'));
    await writeFile(join(path, 'owned'), 'replacement');
