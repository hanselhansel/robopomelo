...
export function brief(doc: ReviewDocument, s: ProjectSnapshot): string {
  return document(doc.title, doc, s);
}
export function acceptance(doc: ReviewDocument, s: ProjectSnapshot): string {
  return (
    document(`${doc.title}: acceptance plan`, doc, s, [
      'acceptanceTests',
      'evidence',
      'readiness',
      'review',
...
export function handoff(doc: ReviewDocument, s: ProjectSnapshot): string {
  return (
    document(`${doc.title}: engineering handoff`, doc, s, [
      'project',
      'needs',
      'problems',
      'workflows',
