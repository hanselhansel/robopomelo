...
export function malformedHeader(): never {
  throw new IngestionError(
    ...
    'The file header is malformed or incomplete.',
    'Export the file again as PDF, PNG or JPEG.',
  );
}
function dimensions(width: number, height: number): ImageDimensions {
  if (width === 0 || height === 0) malformedHeader();
  if (width * height > DEFAULT_INGESTION_LIMITS.maxImagePixels)
    throw new IngestionError(
      'ATTACHMENT_IMAGE_TOO_LARGE',
      'The image exceeds 20 megapixels.',
      'Resize the image before selecting it again.',
    );
  return { width, height };
}
...
export function inspectJpeg(bytes: Uint8Array): ImageDimensions {
  ...
  while (offset < bytes.length) {
    ...
    if (marker === undefined || marker === 0 || marker === 0xd8 || marker === 0xd9 || marker === 0xda)
      malformedHeader();
    ...
    if ([0xc0, 0xc1, 0xc2].includes(marker)) {
      ...
      if (![1, 3, 4].includes(components) || length !== 8 + 3 * components || bytes[offset + 2] !== 8)
        malformedHeader();
      return dimensions(
