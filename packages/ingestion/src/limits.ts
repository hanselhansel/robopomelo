...
export function assertAttachmentBatch(files: readonly { byteLength: number }[]): void {
  if (files.length > DEFAULT_INGESTION_LIMITS.maxFiles)
  ...
  for (const { byteLength } of files) {
    if (!Number.isSafeInteger(byteLength) || byteLength <= 0)
      throw new IngestionError(
        'ATTACHMENT_INVALID_SIZE',
        'The file has no valid content size.',
        'Choose a nonempty file.',
      );
    if (byteLength > DEFAULT_INGESTION_LIMITS.maxFileBytes)
      throw new IngestionError(
        'ATTACHMENT_FILE_TOO_LARGE',
        'A file exceeds 25 MiB.',
        'Export a smaller file and select it again.',
      );
    total += byteLength;
  }
  if (total > DEFAULT_INGESTION_LIMITS.maxTotalBytes)
    throw new IngestionError(
      'ATTACHMENT_TOTAL_TOO_LARGE',
      'The selected files exceed 100 MiB in total.',
      'Remove files or export smaller versions.',
    );
}
export function assertPdfPageCount(count: number): void {
  if (!Number.isSafeInteger(count) || count <= 0)
    throw new IngestionError(
      'ATTACHMENT_INVALID_PAGE_COUNT',
      'The PDF has no valid page count.',
      'Export a valid PDF and select it again.',
    );
  if (count > DEFAULT_INGESTION_LIMITS.maxPdfPages)
    throw new IngestionError(
      'ATTACHMENT_PDF_TOO_MANY_PAGES',
      'The PDF exceeds 100 pages.',
      'Export only the relevant pages.',
    );
}
