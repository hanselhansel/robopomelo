...
export interface ReferenceEntry { id:string; collection:string; path:string; record:Record<string,unknown> }
export function buildReferenceIndex(deployment: Deployment): Map<string,ReferenceEntry> {
  ...
  function visit(value: unknown, collection: string, path: string): void {
    ...
    if (Array.isArray(value)) { value.forEach((item,i) => visit(item,collection,`${path}/${i}`)); return; }
  ...
  for (const [key,value] of Object.entries(deployment)) if (key !== 'extensions' && key !== 'meta') visit(value,key,`/${key}`);
  return index;
}
