...
for(const field of fields){const set=allowlist.get(field.collection)??new Set<string>();set.add(field.path);allowlist.set(field.collection,set);}
...
for(const [collection,advanced] of [['evidence',['location']],['decisions',['actor','decidedAt']],['challengeAnswers',['promptId','promptVersion']]] as const)for(const key of advanced)allowlist.get(collection)!.add(key);
function assignAllowed(target:Record<string,unknown>,collection:Collection|'project',values:Record<string,Json>):void {
 const allowed=allowlist.get(collection);
 for(const key of Object.keys(values))if(!allowed?.has(key)||['__proto__','prototype','constructor','id'].includes(key))throw new DomainError('FIELD_NOT_ALLOWED',`Field is not authorable: ${collection}.${key}`);
...
export function evaluatePatch(d:Deployment,patch:PatchEnvelope,c:PatchContext):PatchEvaluation {
 ...
 for(const op of patch.operations) {
  ...
  if(!allowlist.has(collection))throw new DomainError('COLLECTION_NOT_ALLOWED','Collection is not authorable.');
  const rows=candidate[collection] as RecordBase[];
  if(op.op==='add') {
   const record=structuredClone(op.record) as unknown as RecordBase;
   if(rows.some(r=>r.id===record.id))throw new DomainError('DUPLICATE_ID','Record ID already exists.');
   checkRecordPermissions(collection,undefined,record,c,patch.actor);rows.push(record);
  } else {
   ...
   if(op.op==='remove'){checkRecordPermissions(collection,old,undefined,c,patch.actor);rows.splice(index,1);}
   else {assignAllowed(rows[index]! as unknown as Record<string,unknown>,collection,op.fields);checkRecordPermissions(collection,old,rows[index],c,patch.actor);}
  }
 }
