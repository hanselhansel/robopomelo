...
export function hasValue<T>(
  value: Knowledge<T> | undefined,
): value is Extract<NonNullable<Knowledge<T>>, { state: 'provided' | 'unverified' }> {
  return value != null && (value.state === 'provided' || value.state === 'unverified');
}
export function knowledgeText<T>(
  knowledge: Knowledge<T> | undefined,
  display: (value: T) => string = (value) => String(value),
): string {
