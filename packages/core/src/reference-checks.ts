...
const targets: Record<string, readonly string[]> = {
  ownerId: ['stakeholders'],
  approverId: ['stakeholders'],
  assessorId: ['stakeholders'],
  handoffToId: ['stakeholders'],
  beneficiaryIds: ['stakeholders'],
  affectedStakeholderIds: ['stakeholders'],
  workflowIds: ['workflows'],
  needIds: ['needs'],
  requirementIds: ['requirements'],
  kpiIds: ['kpis'],
  testIds: ['acceptanceTests'],
  assumptionIds: ['assumptions'],
  sourceEvidenceIds: ['evidence'],
  evidenceIds: ['evidence'],
  evidenceRequirementIds: ['evidence'],
  subjectIds: ['requirements', 'kpis', 'workflows'],
};
const authorCollections = new Set([
  'stakeholders',
  'needs',
  'problems',
  'workflows',
...
export function checkReferences(d: Deployment, index: Map<string, ReferenceEntry>, emit: Emit): void {
  ...
  function visit(value: unknown, path: string, owner: string): void {
    ...
    if (typeof obj.id === 'string') owner = obj.id;
    for (const [key, v] of Object.entries(obj)) {
      ...
      if (targets[key] || key === 'relatedIds') {
        if (Array.isArray(v)) v.forEach((id, i) => check(id as string, key, `${path}/${key}/${i}`, owner));
          ...
          check((v as { value: string }).value, key, `${path}/${key}/value`, owner);
  ...
  for (const collection of authorCollections)
    visit(d[collection as keyof Deployment], `/${collection}`, d.project.id);
  ...
  for (const [collection, rows] of [
    ['approvals', d.review.approvals],
    ['revocations', d.review.revocations],
    ['invalidations', d.review.invalidations],
  ] as const)
    rows.forEach((row, i) => {
      if ('approvalId' in row && !d.review.approvals.some((a) => a.id === row.approvalId))
        emit('RP-003', [row.id, row.approvalId], [`/review/${collection}/${i}/approvalId`]);
      if ('acknowledgmentIds' in row)
        for (const [key, ids, available] of [
          ['acknowledgmentIds', row.acknowledgmentIds, d.review.acknowledgments],
          ['waiverIds', row.waiverIds, d.review.waivers],
        ] as const)
          ids.forEach((id) => {
            if (!available.some((a) => a.id === id))
              emit('RP-003', [row.id, id], [`/review/${collection}/${i}/${key}`]);
          });
    });
  if (d.review.currentApprovalId && !d.review.approvals.some((a) => a.id === d.review.currentApprovalId))
    emit('RP-003', [d.review.currentApprovalId], ['/review/currentApprovalId']);
}
