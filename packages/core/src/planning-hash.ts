...
function planningEvidence(d:Deployment):Set<string> {
  ...
  function collect(value:unknown):void {
    ...
    for(const [key,item] of Object.entries(value)){
      ...
      if((key==='sourceEvidenceIds'||key==='evidenceIds')&&Array.isArray(item)) for(const id of item)if(typeof id==='string')selected.add(id);
      collect(item);
    }
  }
  for(const [key,value] of Object.entries(d))if(!['meta','review','extensions','evidence','decisions'].includes(key))collect(value);
  ...
  for(const id of selected){const evidence=byId.get(id);if(evidence)collect(evidence);}
  return selected;
}
...
export function planningHash(deployment: Deployment): string {
  ...
  for (const [key,raw] of Object.entries(deployment)) {
    ...
    if (key === 'evidence') value = deployment.evidence.filter(item => includedEvidence.has(item.id)) as unknown as Json;
