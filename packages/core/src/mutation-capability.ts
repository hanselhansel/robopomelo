...
function supportedRange(version: string, range: string): boolean {
  const current = versionParts(version),
    base = versionParts(range.startsWith('^') ? range.slice(1) : range);
...
export function assertCapabilitySupport(
  capability: Capability | undefined,
  specVersion: string,
): asserts capability is Capability {
  if (
    !capability ||
    capability.kind !== 'skill' ||
    capability.stage !== 'stable' ||
    !capability.available ||
    !supportedRange(specVersion, capability.specRange)
  )
    throw new DomainError(
      'UNSUPPORTED_CAPABILITY',
      'Declared Skill capability is unavailable or does not support this specification.',
    );
}
export function checkCapabilityOperation(op: PatchOperation, writes: readonly string[]): void {
  ...
  const required =
    op.op === 'add' || op.op === 'remove'
      ? [`${collection}.*`]
      : Object.keys(op.fields).map((field) => `${collection}.${field}`);
  for (const field of required)
    if (!writes.includes(field) && !writes.includes(`${collection}.*`))
      throw new DomainError('FIELD_NOT_ALLOWED', `Declared Skill cannot write ${field}.`, { field });
}
export function checkDeclaredCapability(patch: PatchEnvelope, specVersion: string): void {
  ...
  const capability = capabilities.find((c) => c.id === patch.capabilityId);
  assertCapabilitySupport(capability, specVersion);
