...
function failure(limit: Limit, instancePath: string): ErrorObject {
  return {keyword:'inputLimit',instancePath,schemaPath:'#/inputLimits',
    params:{limit,...(limit === 'cycle' ? {} : {maximum:INPUT_LIMITS[limit]})},
    message:limit === 'cycle' ? 'must not contain cyclic references' : `must stay within the ${limit} input limit`};
}
...
export function checkInputLimits(input: unknown): ErrorObject | null {
  ...
  function enter(value: unknown, path: string, depth: number): ErrorObject | null {
    ...
    if (!Array.isArray(value) && Object.hasOwn(value,'id') && ++records > INPUT_LIMITS.records) return failure('records',path);
  ...
  while (frames.length) {
    ...
    const error = enter((frame.value as Record<string,unknown>)[key],`${frame.path}/${pointer(key)}`,frame.depth+1);
