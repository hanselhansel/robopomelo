...
## Implemented offline verification adaptation, 2026-09-05

Installed `sigstore` 5.0.0 calls `@sigstore/tuf.getTrustedRoot` from `dist/sigstore.js`. The installed `tuf-js/dist/updater.js` forceCache path (lines 87-97) fetches root/timestamp on invalid or expired metadata; `@sigstore/tuf/dist/target.js` can download a missing target. Therefore `tufForceCache` cannot satisfy the absolute offline contract. The implementation instead calls the same maintained `@sigstore/verify` Verifier, `bundleFromJSON`, `toSignedEntity` and `toTrustMaterial` directly, with the installed @sigstore/tuf 5.0.0 bundled trusted_root seed. It requires certificate transparency, transparency-log and timestamp thresholds of one, exact issuer and anchored workflow SAN, then checks the authenticated in-toto subject/package/digest and SLSA repository/workflow/ref. It creates no TUF/network client. Unknown or rotated trust fails closed and requires an explicit package-manager update of the installed launcher. Synthetic upstream signed fixtures exercise the cryptographic path without claiming RoboPomelo publication; actual RC/stable publisher provenance remains a release gate.
