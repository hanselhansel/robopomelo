...
[Distribution](../../.github/workflows/distribution.yml) runs native Ubuntu 24.04, Windows 2025 and macOS 15 on current Node 22 and 24 patches, plus macOS 15 Intel on Node 24. Each leg records actual OS, architecture and Node version, runs storage/security/CLI/Skills/update tests and installs the packed standalone package into isolated temporary paths. The package verifier exercises init, validation, exports and loopback HTTP. No Docker or WSL result substitutes for a native leg. Matrix failure does not cancel remaining evidence collection.

Both Windows runtime legs also install and exercise actual Chrome and Edge binaries. The Linux browser job builds the actual package and runs its Chromium, Firefox and WebKit projects, including offline/outbound checks. Browser engines are installed only into the test environment. Hosted Chrome and Edge outcomes require actual Windows runs. Native Safari and VoiceOver acceptance remain separate evidence. WebKit is not recorded as Safari. HTML reports and runtime results are uploaded even after a failing test; absent artifacts fail the upload.
