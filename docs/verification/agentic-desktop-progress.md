...
D3 in progress:83ad15c adds preflight; a9359cc adds the reviewed sandbox parser. Native selected-file I/O, the bounded broker and authenticated preview downloads are now connected. Real application smoke covers selection, cancelled chooser, in-page navigation, parsing, PNG fetch and revocation on both normal quit paths. Scoped reviews closed stale chooser/navigation/project-context races with red-green regressions; final quality recheck passed17nativebridge tests. Root's last focused run passed39file/broker/preview/native tests plus typecheck; the rebuilt CLI passed all9installed-package checks after the shared-service hook change. See [attachment intake verification](attachment-ingestion.md). Next work is the retained native intake UI and D4 permission/immutable-import persistence. No provider, GPU or signing gate has changed.
