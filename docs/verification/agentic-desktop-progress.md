...
D3 in progress:83ad15c adds pure byte/header preflight and limits with10green tests. The separate sandbox parser now passes actual Electron44.2.0 decode/isolation/cancellation/deadline checks, including password-protected and named-CMap PDFs and exit of a demonstrably hung renderer process. Scoped spec review closed a CMap configuration gap after a real red-green fixture; scoped quality review found no remaining actionable issues and independently passed16parser/ingestion tests. Root's combined43desktop/ingestion tests,61tooling tests and existing application/isolation/parser smoke sequence passed. See [attachment intake verification](attachment-ingestion.md). Next work is the main-owned selected-file broker and preview protocol, then retained native intake UI and D4 grants. No provider, GPU or signing gate has changed.
